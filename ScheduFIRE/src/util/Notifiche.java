...
public class Notifiche {
	...
	public Notifiche() {
		...
		listaNotifiche.add(new Notifica(1, "Test di una notifica normale, che per quanto sia normale essa Ë solo una notifica normale, dunque Ë normale che sia normale", "HomeCTServlet", generateId()));
	...
	private static void firstUpdate() {
		...
		if(!conta(disponibili)) {
			...
			listaNotifiche.add(new Notifica(3, "Il personale disponibile il " + 
					data.toString() + " non ÔøΩ sufficiente per creare il turno.", "/GestionePersonaleServlet",generateId()));
		}
	}
	...
	/**
	 * Utilizzare quando vengono concesse ferie ad un vigile gi√† schedulato
	 */
